...
import { savingsMenuData, SavingsCategory } from "@/data/savingsMenuData";

interface SavingsMegaMenuProps {
  onClose: () => void;
}
...
const getIcon = (iconName: string) => {
  switch (iconName) {
    case "piggy-bank":
      return <PiggyBank className="w-6 h-6 text-primary" />;
    case "wallet":
...
const SavingsMegaMenu = ({ onClose }: SavingsMegaMenuProps) => {
  return (
    <div className="absolute top-full right-0 w-[700px] pt-2 z-50">
      ...
      >
        <div className="grid grid-cols-3 gap-8">
          {savingsMenuData.map((category: SavingsCategory) => (
        ...
        <div className="mt-6 bg-gradient-to-l from-muted/50 to-muted rounded-xl p-4 flex items-center gap-4">
          <div className="flex-1">
            <p className="text-sm text-muted-foreground">מה מצב החסכונות שלך?</p>
            <p className="font-bold text-foreground">המומחים שלנו יעשו לך סדר</p>
          </div>
          <a
            href="/contact"
...
export default SavingsMegaMenu;
