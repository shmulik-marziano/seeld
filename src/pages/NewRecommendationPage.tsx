  ...
  return (
    <div className="max-w-3xl mx-auto animate-fade-in px-0">
      ...
      <div className="flex items-center gap-2 md:gap-3 mb-4 md:mb-6">
        <Button variant="ghost" size="sm" onClick={goBack} className="hover:bg-[#003D30]/10 shrink-0 h-9 w-9 p-0 md:h-10 md:w-10 rounded-xl min-h-[44px]">
        ...
        {allSteps.map((s, i) => (
          <div key={s} className="flex-1 relative">
            <div className={`h-1.5 md:h-2 rounded-full transition-all duration-500 ease-out ${
              i < stepNumber
                ? 'bg-gradient-to-l from-[#003D30] to-[#003D30] shadow-sm shadow-[#003D30]/20'
                : i === stepNumber - 1
                  ? 'bg-[#003D30]'
                  : 'bg-border/50'
            }`} />
      ...
      {step === 'choose-customer' && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
          <Card className="cursor-pointer hover:border-[#003D30]/50 hover:shadow-xl hover:shadow-[#003D30]/5 transition-all duration-300 group rounded-2xl hover:-translate-y-1" onClick={() => handleChooseMode('existing')}>
            <CardContent className="flex flex-col items-center gap-2 md:gap-3 p-6 md:p-10">
              <div className="w-14 h-14 md:w-18 md:h-18 rounded-2xl bg-[#003D30]/8 flex items-center justify-center group-hover:bg-[#003D30]/15 group-hover:shadow-lg group-hover:shadow-[#003D30]/10 transition-all duration-300">
                <Users className="h-7 w-7 md:h-8 md:w-8 text-[#003D30]" />
              </div>
          ...
          <Card className="cursor-pointer hover:border-[#003D30]/50 hover:shadow-xl hover:shadow-[#003D30]/5 transition-all duration-300 group rounded-2xl hover:-translate-y-1" onClick={() => handleChooseMode('new')}>
            <CardContent className="flex flex-col items-center gap-2 md:gap-3 p-6 md:p-10">
              <div className="w-14 h-14 md:w-18 md:h-18 rounded-2xl bg-[#003D30]/10 flex items-center justify-center group-hover:bg-[#003D30]/20 group-hover:shadow-lg group-hover:shadow-[#003D30]/10 transition-all duration-300">
                <UserPlus className="h-7 w-7 md:h-8 md:w-8 text-[#003D30]" />
              </div>
      ...
      {step === 'customer-details' && customerMode === 'existing' && (
        <Card className="rounded-2xl border-border/50 shadow-sm overflow-hidden">
          <CardHeader className="bg-gradient-to-b from-[#003D30]/3 to-transparent">
            <CardTitle className="text-lg flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-xl bg-[#003D30]/10 flex items-center justify-center">
                <Search className="h-4 w-4 text-[#003D30]" />
              </div>
              חיפוש לקוח
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4 p-4 md:p-6">
            <Input placeholder="חפש לפי שם, ת.ז או טלפון..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} autoFocus
              className="rounded-xl h-12 border-border/60 focus:border-[#003D30] transition-colors text-sm" />
            <div className="space-y-1 max-h-80 overflow-y-auto">
              ...
              {filteredCustomers.map(c => (
                <button key={c.id} onClick={() => handleSelectExistingCustomer(c.id)}
                  className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-[#003D30]/5 transition-all duration-200 text-right group min-h-[52px]">
                  <div className="w-10 h-10 rounded-xl bg-[#003D30]/8 flex items-center justify-center text-[#003D30] font-bold text-sm group-hover:bg-[#003D30]/12 group-hover:shadow-md transition-all duration-200">
                    {c.firstName[0]}{c.lastName[0]}
                  </div>
                  <div className="flex-1">
                    <p className="text-sm font-medium group-hover:text-[#003D30] transition-colors">{c.fullName}</p>
                    <p className="text-xs text-muted-foreground">{c.idNumber} · {c.mobilePhone}</p>
                  </div>
                  <ArrowLeft className="h-4 w-4 text-muted-foreground/40 group-hover:text-[#003D30] group-hover:translate-x-[-2px] transition-all duration-200" />
                </button>
              ))}
            </div>
          </CardContent>
        ...
        <div className="space-y-6">
          <Card className="rounded-2xl border-border/50 shadow-sm overflow-hidden">
            <CardHeader className="bg-gradient-to-b from-[#003D30]/3 to-transparent">
              <CardTitle className="text-lg flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-xl bg-[#003D30]/10 flex items-center justify-center">
                  <UserPlus className="h-4 w-4 text-[#003D30]" />
                </div>
                פרטי לקוח חדש
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4 p-4 md:p-6">
              <div className="grid grid-cols-2 gap-3 md:gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input value={form.firstName} onChange={e => setForm(p => ({ ...p, firstName: e.target.value }))} className="rounded-xl h-11 border-border/60 focus:border-[#003D30]" />
                  {errors.firstName && <p className="text-xs text-destructive mt-1">{errors.firstName}</p>}
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs font-medium text-muted-foreground">שם משפחה *</Label>
                  <Input value={form.lastName} onChange={e => setForm(p => ({ ...p, lastName: e.target.value }))} className="rounded-xl h-11 border-border/60 focus:border-[#003D30]" />
              ...
              <div className="grid grid-cols-2 gap-3 md:gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input value={form.idNumber} onChange={e => setForm(p => ({ ...p, idNumber: e.target.value }))} className="rounded-xl h-11 border-border/60 focus:border-[#003D30]" />
                ...
                <div className="space-y-1.5">
                  ...
                  <Input value={form.mobilePhone} onChange={e => setForm(p => ({ ...p, mobilePhone: e.target.value }))} className="rounded-xl h-11 border-border/60 focus:border-[#003D30]" />
              ...
              <div className="space-y-1.5">
                ...
                <Input value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} type="email" className="rounded-xl h-11 border-border/60 focus:border-[#003D30]" />
              </div>
            </CardContent>
          </Card>
          <Button onClick={handleCreateAndContinue} size="lg" className="gap-2 rounded-2xl bg-[#003D30] hover:bg-[#003D30]/90 shadow-lg shadow-[#003D30]/20 min-h-[48px] transition-all duration-200 hover:shadow-xl hover:-translate-y-0.5" disabled={saving}>
      ...
      {step === 'manage-products' && (
        <div className="space-y-4">
          {selectedCustomer && (
            <div className="flex items-center gap-3 p-3.5 bg-gradient-to-l from-[#003D30]/5 to-[#003D30]/5 rounded-2xl mb-4 border border-[#003D30]/10">
              <div className="w-10 h-10 rounded-xl bg-[#003D30]/10 flex items-center justify-center text-[#003D30] font-bold text-sm shadow-sm">
            ...
            <CardContent className="p-4 md:p-6">
              <div
                className={`border-2 border-dashed rounded-2xl p-5 text-center cursor-pointer transition-all duration-300 ${
                  fileDragging ? 'border-[#003D30] bg-[#003D30]/10 scale-[1.01]' :
                  fileUploading ? 'border-[#003D30] bg-[#003D30]/5' : 'border-border/60 hover:border-[#003D30]/50 hover:bg-[#003D30]/5'
                }`}
                ...
                {fileUploading ? (
                  <div className="flex flex-col items-center gap-3 py-2">
                    <div className="relative">
                      <div className="w-12 h-12 rounded-full border-2 border-[#003D30]/20 flex items-center justify-center">
                        <Loader2 className="h-6 w-6 animate-spin text-[#003D30]" />
                      </div>
                      <div className="absolute inset-0 w-12 h-12 rounded-full border-2 border-transparent border-t-[#003D30] animate-spin" style={{ animationDuration: '1.5s' }} />
                    </div>
                    <p className="text-sm font-bold text-[#003D30]">{fileUploadStatus}</p>
                    <p className="text-xs text-muted-foreground">מוצרי רכב ודירה מסוננים אוטומטית</p>
                  </div>
                ) : (
                  <>
                    <div className="w-12 h-12 rounded-2xl bg-[#003D30]/8 flex items-center justify-center mx-auto mb-2">
                      <Upload className="h-5 w-5 text-[#003D30]/60" />
                    </div>
          ...
          <Card className="rounded-2xl border-border/50 shadow-sm overflow-hidden">
            <CardHeader className="bg-gradient-to-b from-[#003D30]/3 to-transparent">
              <div className="flex items-center justify-between">
                <CardTitle className="text-lg flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-xl bg-[#003D30]/15 flex items-center justify-center">
                    <Package className="h-4 w-4 text-[#003D30]" />
                  </div>
                  מוצרים ({customerProducts.length})
                </CardTitle>
                <Button size="sm" variant="outline" className="gap-1.5 rounded-xl min-h-[36px] border-[#003D30]/20 text-[#003D30] hover:bg-[#003D30]/5" onClick={() => { setEditingProduct(undefined); setProductModalOpen(true); }}>
            ...
            <CardContent className="space-y-3">
              {customerProducts.length === 0 ? (
                ...
                <div className="space-y-3">
                  ...
                  {customerProducts.filter(p => p.category === 'כסף').length > 0 && (
                    <div className="border rounded-lg overflow-hidden">
                      <div className="flex items-center gap-2 px-3 py-2 bg-gold/5 border-b">
                        <span>💰</span>
                        <span className="text-xs font-bold text-muted-foreground">כסף ({customerProducts.filter(p => p.category === 'כסף').length})</span>
                      </div>
                  ...
                  {customerProducts.filter(p => p.category === 'ביטוח').length > 0 && (
                    <div className="border rounded-lg overflow-hidden">
                      <div className="flex items-center gap-2 px-3 py-2 bg-info/5 border-b">
                        <span>🛡️</span>
                        <span className="text-xs font-bold text-muted-foreground">ביטוח ({customerProducts.filter(p => p.category === 'ביטוח').length})</span>
                      </div>
                      <div className="divide-y divide-border/30">
                  ...
                  {/* Progress indicator */}
                  <div className="flex items-center gap-3 p-3.5 rounded-2xl bg-gradient-to-l from-[#003D30]/5 to-[#003D30]/5 border border-[#003D30]/10">
                    <div className="flex-1">
                      <div className="flex justify-between text-xs mb-1.5">
                        <span className="text-muted-foreground font-medium">המלצות נוצרו</span>
                        <span className="font-bold text-[#003D30]">{recDrafts.length} / {customerProducts.length}</span>
                      </div>
                      <div className="h-2 bg-[#003D30]/10 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-gradient-to-l from-[#003D30] to-[#003D30] rounded-full transition-all duration-500 ease-out shadow-sm shadow-[#003D30]/30"
          ...
          <div className="flex gap-3">
            <Button onClick={handleContinueToRecs} size="lg" className="gap-2 rounded-2xl bg-[#003D30] hover:bg-[#003D30]/90 shadow-lg shadow-[#003D30]/20 min-h-[48px] transition-all duration-200 hover:shadow-xl hover:-translate-y-0.5" disabled={customerProducts.length === 0}>
      ...
      {step === 'write-recommendations' && (
        <div className="space-y-6">
          <div className="flex items-center gap-3 p-3.5 bg-gradient-to-l from-[#003D30]/5 to-[#003D30]/5 rounded-2xl border border-[#003D30]/10">
            {selectedCustomer && (
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 rounded-lg bg-[#003D30]/10 flex items-center justify-center text-[#003D30] font-bold text-[10px]">
                  {selectedCustomer.firstName[0]}{selectedCustomer.lastName[0]}
                </div>
                <span className="font-bold text-sm">{selectedCustomer.fullName}</span>
              </div>
            )}
                ...
                <CardHeader className="pb-3 px-3 md:px-6 bg-gradient-to-b from-muted/30 to-transparent">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <CardTitle className="text-sm md:text-base flex items-center gap-2">
                      <span className="w-6 h-6 md:w-7 md:h-7 rounded-xl bg-[#003D30] text-white text-[10px] md:text-xs flex items-center justify-center font-bold shrink-0 shadow-sm shadow-[#003D30]/20">
                        {idx + 1}
                      </span>
                    ...
                    <div className="flex gap-1.5 shrink-0">
                      <Button variant="outline" size="sm"
                        className="gap-1 text-[10px] md:text-xs h-8 px-2.5 rounded-xl border-[#003D30]/30 text-[#003D30] hover:bg-[#003D30]/10 min-h-[32px] transition-all duration-200"
                        onClick={() => generateAiRecommendation(draft.productId)}
                        disabled={aiLoading[draft.productId]}>
                        {aiLoading[draft.productId] ? (
                          <div className="relative">
                            <Sparkles className="h-3 w-3 animate-pulse" />
                            <div className="absolute inset-0 w-3 h-3 rounded-full border border-transparent border-t-[#003D30] animate-spin" />
                          </div>
          ...
          <div className="flex gap-3">
            <Button onClick={() => setStep('preview')} size="lg" className="gap-2 rounded-2xl bg-[#003D30] hover:bg-[#003D30]/90 shadow-lg shadow-[#003D30]/20 min-h-[48px] transition-all duration-200 hover:shadow-xl hover:-translate-y-0.5">
      ...
      {step === 'preview' && (
        <div className="space-y-6">
          <Card className="border-[#003D30]/20 bg-gradient-to-l from-[#003D30]/5 to-[#003D30]/5 rounded-2xl">
            <CardContent className="p-4 flex items-center justify-center gap-2">
              <Eye className="h-4 w-4 text-[#003D30]" />
              <p className="text-sm text-center text-[#003D30] font-medium">תצוגה מקדימה — כך הלקוח יראה את ההמלצות</p>
            </CardContent>
          </Card>
                ...
                <CardContent className="space-y-3">
                  {draft.currentState && (
                    <div className="bg-muted/50 rounded-lg p-3">
                      <h4 className="text-xs font-bold text-muted-foreground mb-1">מצב קיים</h4>
                      <p className="text-sm">{draft.currentState}</p>
                    </div>
                  )}
                  {draft.problemGap && (
                    <div className="bg-warning/5 rounded-lg p-3 border border-warning/10">
                      <h4 className="text-xs font-bold text-warning mb-1">המלצת סוכן</h4>
                      <p className="text-sm">{draft.problemGap}</p>
                    </div>
                  )}
                  <div>
                    <h4 className="text-xs font-bold text-muted-foreground mb-1">נימוק</h4>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {draft.advantages && (
                        <div className="bg-success/5 rounded-lg p-3 border border-success/10">
                          <h4 className="text-xs font-bold text-success mb-1">יתרונות</h4>
                          <p className="text-sm">{draft.advantages}</p>
                        </div>
                      )}
                      {draft.disadvantages && (
                        <div className="bg-destructive/5 rounded-lg p-3 border border-destructive/10">
                          <h4 className="text-xs font-bold text-destructive mb-1">חסרונות</h4>
                  ...
                  {draft.improvement && (
                    <div className="bg-success/5 rounded-lg p-3 border border-success/10">
                      <h4 className="text-xs font-bold text-success mb-1">מה משתפר</h4>
          ...
          <div className="flex gap-3">
            <Button onClick={handleSubmitAll} size="lg" className="gap-2 rounded-2xl bg-[#003D30] hover:bg-[#003D30]/90 shadow-lg shadow-[#003D30]/20 min-h-[48px] transition-all duration-200 hover:shadow-xl hover:-translate-y-0.5" disabled={saving}>
              {saving && <Loader2 className="h-4 w-4 animate-spin" />}
      ...
      {step === 'summary' && (
        <div className="space-y-6">
          ...
          <Card className="rounded-2xl border-border/50 shadow-sm overflow-hidden">
            <CardHeader className="bg-gradient-to-b from-[#003D30]/3 to-transparent">
              <div className="flex items-center justify-between">
                <CardTitle className="text-lg flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-xl bg-[#003D30]/10 flex items-center justify-center">
                    <Link2 className="h-4 w-4 text-[#003D30]" />
                  </div>
                  שליחה ונעילה
                </CardTitle>
                <Button size="sm" className="gap-1.5 rounded-xl bg-[#003D30] hover:bg-[#003D30]/90 min-h-[36px]" onClick={handleSendSummary}>
          ...
          <div className="flex gap-2 md:gap-3 flex-wrap">
            ...
            <Button onClick={() => navigate('/app/recommendation-bank')} size="lg" className="rounded-2xl min-h-[48px] bg-[#003D30] hover:bg-[#003D30]/90 shadow-lg shadow-[#003D30]/20">
              עבור לבנק המלצות
            </Button>
...
function WizardProductRow({ product, hasDraft, onAddDraft, onEdit, onDelete }: {
  ...
  return (
    <div className={`flex items-center gap-3 px-3 py-3 transition-all duration-200 ${hasDraft ? 'bg-[#003D30]/5' : 'hover:bg-muted/30'}`}>
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        {hasDraft ? (
          <Badge variant="secondary" className="text-[10px] gap-1 bg-[#003D30]/10 text-[#003D30] border-[#003D30]/15 rounded-lg">
            <CheckCircle2 className="h-3 w-3" />יש המלצה
          </Badge>
        ) : (
          <Button variant="outline" size="sm" className="text-[10px] gap-1 h-8 px-2.5 border-[#003D30]/20 text-[#003D30] hover:bg-[#003D30]/5 rounded-xl min-h-[32px]" onClick={onAddDraft}>
