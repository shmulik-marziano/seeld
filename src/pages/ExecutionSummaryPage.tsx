  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-5" dir="rtl">
      ...
      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
        <button onClick={() => navigate('/')} className="hover:text-primary transition-colors">ראשי</button>
        <span className="text-muted-foreground/50">‹</span>
        <button onClick={() => navigate('/app/customers')} className="hover:text-primary transition-colors">לקוחות</button>
        <span className="text-muted-foreground/50">‹</span>
        <button onClick={() => navigate(`/customers/${customerId}`)} className="hover:text-primary transition-colors">{customer.fullName}</button>
        <span className="text-muted-foreground/50">‹</span>
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-xl md:text-2xl font-bold">סיכום ביצועים</h1>
              {activeSummary && (
        ...
        <div className="flex gap-2">
          {activeSummary && (
            <>
              <Button variant="outline" size="sm" onClick={handleShareWhatsApp} className="gap-1.5 text-success border-success/30 hover:bg-success/5">
                <MessageCircle className="h-3.5 w-3.5" />WhatsApp
              </Button>
              <Button variant="outline" size="sm" onClick={handleShareLink} className="gap-1.5">
                <Share2 className="h-3.5 w-3.5" />העתק לינק
              </Button>
              <Button variant="outline" size="sm" onClick={handleExportPdf} className="gap-1.5">
                <Download className="h-3.5 w-3.5" />PDF
              </Button>
              <Button variant="outline" size="sm" onClick={() => setShowPresentation(true)} className="gap-1.5">
                <Presentation className="h-3.5 w-3.5" />מצגת
              </Button>
            ...
            <CardContent className="p-4">
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium">{SUMMARY_STATUS_LABELS[activeSummary.status as ExecutionSummaryStatus]}</p>
                </div>
                <div>
                  <p className="text-muted-foreground text-xs">בוצע מלא</p>
                  <p className="font-semibold text-success">{completedCount}/{totalItems}</p>
                </div>
                <div>
                  <p className="text-muted-foreground text-xs">בוצע חלקית</p>
                  <p className="font-medium text-warning">{partialCount}</p>
                </div>
                <div>
                  <p className="text-muted-foreground text-xs">לא בוצע</p>
                  <p className="font-medium text-destructive">{totalItems - completedCount - partialCount}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            {summaryItems.map((si, idx) => {
              ...
              return (
                <motion.div key={si.id} variants={item} initial="hidden" animate="show" transition={{ delay: idx * 0.05 }}>
                  <Card className={`transition-all ${si.executionStatus === 'fully_executed' ? 'border-success/30 bg-success/5' : si.executionStatus === 'partially_executed' ? 'border-warning/30 bg-warning/5' : ''}`}>
                        ...
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                              si.executionStatus === 'fully_executed' ? 'bg-success/10 text-success' :
                              si.executionStatus === 'partially_executed' ? 'bg-warning/10 text-warning' :
