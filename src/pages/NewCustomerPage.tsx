...
export default function NewCustomerPage() {
  ...
  const handleDrop = (type: 'maslaqa' | 'harBituah', e: React.DragEvent) => {
    e.preventDefault();
    if (type === 'maslaqa') setDraggingMaslaqa(false);
    else setDraggingHar(false);
  ...
  const FileUploadZone = ({ label, type, fileList, dragging, setDrag, inputRef, accept }: {
    ...
    <div
      className={`border-2 border-dashed rounded-2xl p-6 md:p-8 text-center cursor-pointer transition-all duration-300 ${
        dragging ? 'border-[#171717] bg-[#171717]/10 scale-[1.02]' :
        fileList.length > 0 ? 'border-emerald-400/50 bg-emerald-50/30 dark:bg-emerald-900/10' : 'border-border/60 hover:border-[#171717]/50 hover:bg-[#171717]/5'
      }`}
      ...
      >
        <div className="flex items-center gap-3 mb-4">
          <Button
            ...
            className="hover:bg-[#171717]/10 rounded-xl h-10 w-10 p-0"
          >
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 flex-1">
            <div className={`w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold transition-all duration-300 ${
              step >= 1 ? 'bg-[#171717] text-white shadow-md shadow-[#171717]/20' : 'bg-muted text-muted-foreground'
            }`}>1</div>
            <span className={`text-xs font-medium hidden sm:inline ${step >= 1 ? 'text-[#171717]' : 'text-muted-foreground'}`}>העלאת קבצים</span>
          </div>
          <div className={`h-[2px] flex-1 rounded-full transition-all duration-500 ${step >= 2 ? 'bg-[#171717]' : 'bg-border'}`} />
          <div className="flex items-center gap-2 flex-1 justify-end">
            <span className={`text-xs font-medium hidden sm:inline ${step >= 2 ? 'text-[#171717]' : 'text-muted-foreground'}`}>פרטי לקוח</span>
            <div className={`w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold transition-all duration-300 ${
              step >= 2 ? 'bg-[#171717] text-white shadow-md shadow-[#171717]/20' : 'bg-muted text-muted-foreground'
            }`}>2</div>
          </div>
        </div>
      </motion.div>
      ...
      {step === 1 ? (
        ...
        >
          <Card className="rounded-2xl border-border/50 shadow-sm overflow-hidden">
            <CardHeader className="bg-gradient-to-b from-[#171717]/3 to-transparent pb-4">
          ...
          <div className="flex justify-start gap-3">
            <Button
              ...
              size="lg"
              className="gap-2 rounded-2xl bg-[#171717] hover:bg-[#171717]/90 shadow-lg shadow-[#171717]/20 min-h-[48px] transition-all duration-200 hover:shadow-xl hover:-translate-y-0.5"
            >
        ...
        >
            ...
            <CardContent className="space-y-4 p-4 md:p-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input value={form.firstName} onChange={e => updateField('firstName', e.target.value)} placeholder="שם פרטי" className="rounded-xl h-11 border-border/60 focus:border-[#171717] transition-colors" />
                  {errors.firstName && <p className="text-xs text-destructive mt-1">{errors.firstName}</p>}
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs font-medium text-muted-foreground">שם משפחה *</Label>
                  <Input value={form.lastName} onChange={e => updateField('lastName', e.target.value)} placeholder="שם משפחה" className="rounded-xl h-11 border-border/60 focus:border-[#171717] transition-colors" />
                  {errors.lastName && <p className="text-xs text-destructive mt-1">{errors.lastName}</p>}
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs font-medium text-muted-foreground">תעודת זהות *</Label>
                  <Input value={form.idNumber} onChange={e => updateField('idNumber', e.target.value)} placeholder="מספר ת.ז" className="rounded-xl h-11 border-border/60 focus:border-[#171717] transition-colors" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input type="date" value={form.birthDate} onChange={e => updateField('birthDate', e.target.value)} className="rounded-xl h-11 border-border/60 focus:border-[#171717] transition-colors" />
                </div>
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input type="number" inputMode="numeric" min="0" value={form.numberOfChildren} onChange={e => updateField('numberOfChildren', e.target.value)} placeholder="0" className="rounded-xl h-11 border-border/60 focus:border-[#171717] transition-colors" />
                </div>
              </div>
              {form.maritalStatus === 'נשוי/אה' && (
                ...
                >
                  <div className="space-y-1.5">
                    ...
                    <Input value={form.spouseName} onChange={e => updateField('spouseName', e.target.value)} placeholder="שם מלא" className="rounded-xl h-11 border-border/60 focus:border-[#171717] transition-colors" />
                  </div>
                </motion.div>
              )}
            </CardContent>
          ...
          {/* Contact info */}
          <Card className="rounded-2xl border-border/50 shadow-sm overflow-hidden">
            <CardHeader className="pb-3 bg-gradient-to-b from-[#171717]/5 to-transparent">
              <CardTitle className="flex items-center gap-2.5 text-base">
                <div className="w-7 h-7 rounded-lg bg-[#171717]/15 flex items-center justify-center">
                  <Phone className="h-3.5 w-3.5 text-[#171717]" />
                </div>
                פרטי קשר
              </CardTitle>
            </CardHeader>
            <CardContent className="p-4 md:p-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input value={form.mobilePhone} onChange={e => updateField('mobilePhone', e.target.value)} placeholder="05XXXXXXXX" className="rounded-xl h-11 border-border/60 focus:border-[#171717] transition-colors" />
                ...
                <div className="space-y-1.5">
                  ...
                  <Input value={form.email} onChange={e => updateField('email', e.target.value)} placeholder="email@example.com" type="email" className="rounded-xl h-11 border-border/60 focus:border-[#171717] transition-colors" />
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4 md:p-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input value={form.city} onChange={e => updateField('city', e.target.value)} placeholder="עיר" className="rounded-xl h-11 border-border/60 focus:border-[#171717] transition-colors" />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs font-medium text-muted-foreground">רחוב</Label>
                  <Input value={form.street} onChange={e => updateField('street', e.target.value)} placeholder="שם רחוב" className="rounded-xl h-11 border-border/60 focus:border-[#171717] transition-colors" />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs font-medium text-muted-foreground">מספר בית</Label>
                  <Input value={form.houseNumber} onChange={e => updateField('houseNumber', e.target.value)} placeholder="מספר" className="rounded-xl h-11 border-border/60 focus:border-[#171717] transition-colors" />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs font-medium text-muted-foreground">דירה</Label>
                  <Input value={form.apartmentNumber} onChange={e => updateField('apartmentNumber', e.target.value)} placeholder="מספר" className="rounded-xl h-11 border-border/60 focus:border-[#171717] transition-colors" />
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4 p-4 md:p-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                <div className="space-y-1.5">
                  ...
                  <Input value={form.occupation} onChange={e => updateField('occupation', e.target.value)} placeholder="עיסוק" className="rounded-xl h-11 border-border/60 focus:border-[#171717] transition-colors" />
                </div>
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input type="number" inputMode="decimal" value={form.monthlyIncome} onChange={e => updateField('monthlyIncome', e.target.value)} placeholder="₪" className="rounded-xl h-11 border-border/60 focus:border-[#171717] transition-colors" />
                </div>
                <div className="space-y-1.5">
          ...
          <Card className="rounded-2xl border-border/50 shadow-sm overflow-hidden">
            ...
            <CardContent className="p-4 md:p-6">
              <Textarea value={form.internalNotes} onChange={e => updateField('internalNotes', e.target.value)}
                placeholder="הערות פנימיות על הלקוח..." rows={3} className="rounded-xl border-border/60 focus:border-[#171717] transition-colors resize-none" />
            </CardContent>
          </Card>
