import InsurancePageTemplate from '@/components/InsurancePageTemplate';
import { Shield, Heart, Stethoscope, Pill, Building2, FileCheck } from "lucide-react";

const coverageTypes = [
  {
    title: "בסיס",
    icon: Shield,
    items: [
      "תרופות מחוץ לסל הבריאות",
      "השתלות וטיפולים מיוחדים מחוץ לישראל",
      "ניתוחים וטיפולים מחליפי ניתוח מחוץ לישראל",
    ],
  },
  {
    title: "ניתוחים בארץ",
    icon: Building2,
    items: [
      "משלים שב\"ן ללא השתתפות עצמית",
      "שקל ראשון",
      "משלים שב\"ן עם השתתפות עצמית 5,000 ₪",
    ],
  },
  {
    title: "ייעוץ ובדיקות",
    icon: Stethoscope,
    items: [
      "ייעוץ ובדיקות בסיס",
      "ייעוץ ובדיקות מורחב",
    ],
  },
  {
    title: "הרחבות",
    icon: Pill,
    items: [
      "אבחון רפואי מהיר",
      "טיפולים בטכנולוגיות מתקדמות ואביזרים רפואיים",
      "טיפולים ואבחונים לילד",
      "ליווי רפואי וטיפולים אגב אירוע רפואי משמעותי",
    ],
  },
  {
    title: "כתבי שירות",
    icon: FileCheck,
    items: [
      "רפואה אישית און ליין פלוס",
      "רפואה משלימה",
      "ביקור רופא בבית",
    ],
  },
];

const faqItems = [
  {
    q: "האם ביטוח בריאות פרטי מחליף את קופת החולים?",
    a: "לא. ביטוח בריאות פרטי משלים את השירותים של קופת החולים ומעניק כיסוי לטיפולים, תרופות וניתוחים שאינם כלולים בסל הבריאות הממלכתי.",
  },
  {
    q: "מהי תקופת אכשרה?",
    a: "תקופת אכשרה היא פרק זמן (בדרך כלל 3-12 חודשים) מרגע הצטרפות לביטוח ועד שניתן לממש את הכיסוי. תקופה זו משתנה בין חברות ובין סוגי כיסויים.",
  },
  {
    q: "האם אפשר לבטח את כל המשפחה?",
    a: "בהחלט. רוב חברות הביטוח מציעות פוליסות משפחתיות במחיר מוזל, הכוללות כיסוי להורים ולילדים תחת פוליסה אחת.",
  },
  {
    q: "כמה עולה ביטוח בריאות פרטי?",
    a: "העלות משתנה בהתאם לגיל, מצב בריאותי, רמת הכיסוי וחברת הביטוח. באופן כללי, מדובר בעשרות עד מאות שקלים בחודש. ככל שמצטרפים צעירים יותר, הפרמיה נמוכה יותר.",
  },
];

const keyPoints = [
  {
    title: "למה זה חשוב?",
    description: "מערכת הבריאות הציבורית מוגבלת. ביטוח פרטי מבטיח גישה מהירה לרופאים מומחים, בדיקות מתקדמות וזמני המתנה קצרים.",
    icon: Stethoscope,
  },
  {
    title: "מתי לרכוש?",
    description: "כמה שיותר מוקדם, יותר טוב. פרמיה נמוכה בגיל צעיר, תנאי קבלה קלים, וללא חריגים על מצב רפואי קיים.",
    icon: Heart,
  },
  {
    title: "מה חשוב לדעת?",
    description: "בדקו תקופת אכשרה, חריגים, השתתפות עצמית ורשת רופאים. השוואה בין חברות חוסכת מאות שקלים בחודש.",
    icon: Shield,
  },
];

const articles = [
  {
    title: "למה ביטוח בריאות פרטי חשוב?",
    paragraphs: [
      "מערכת הבריאות הציבורית בישראל מספקת שירותים בסיסיים, אך לעיתים קרובות זמני ההמתנה ארוכים, הבחירה ברופא מוגבלת, ותרופות חדשניות אינן כלולות בסל הבריאות. ביטוח בריאות פרטי מאפשר לכם לקבל טיפול רפואי בזמן הנכון, עם גישה לרופאים מומחים, טכנולוגיות מתקדמות ותרופות שאינן בסל.",
      "במקרה של מחלה או ניתוח, ביטוח בריאות פרטי יכול לחסוך לכם חודשים של המתנה ולהעניק לכם שליטה מלאה על הטיפול שלכם. מדובר בהשקעה בבריאות שלכם ושל משפחתכם, שמשתלמת במיוחד ברגעי האמת.",
    ],
  },
  {
    title: "מתי כדאי לרכוש ביטוח בריאות?",
    paragraphs: [
      "התשובה הקצרה היא: כמה שיותר מוקדם, יותר טוב. ככל שמצטרפים בגיל צעיר יותר, הפרמיה החודשית נמוכה יותר ותנאי הקבלה קלים יותר. חשוב במיוחד לרכוש ביטוח בריאות לפני שמתפתחות בעיות רפואיות, מכיוון שמצב רפואי קיים עלול להוביל לחריגים בפוליסה.",
      "מומלץ לרכוש ביטוח בריאות בשלבי חיים משמעותיים: כניסה לשוק העבודה, לידת ילד ראשון, או מעבר לגיל 40. אלו הרגעים שבהם הצורך בכיסוי רפואי איכותי הופך לקריטי.",
    ],
  },
  {
    title: "מה חשוב לדעת לפני שרוכשים?",
    paragraphs: [
      "בבחירת ביטוח בריאות, חשוב לשים לב למספר נקודות מפתח: תקופת אכשרה (תקופת המתנה לפני שניתן לממש את הביטוח), חריגים בפוליסה, גובה ההשתתפות העצמית, ורשת הרופאים והמוסדות הרפואיים הזמינים.",
      "ב-SEELD אנחנו משווים עבורכם בין כל חברות הביטוח בשוק ומוצאים פוליסה שמתאימה לצרכים ולתקציב שלכם, ללא עלות נוספת.",
    ],
  },
];

const HealthInsurance = () => {
  return (
    <InsurancePageTemplate
      heroIcon={Heart}
      heroIconColor="#b91c1c"
      heroCategory="ביטוח בריאות"
      heroTitle="ביטוח בריאות פרטי"
      heroDescription="גישה מהירה לרופאים מומחים, ניתוחים וטיפולים מתקדמים. הגנה מקיפה על הבריאות שלכם ושל משפחתכם."
      heroCTAText="קבלו הצעה מותאמת"
      breadcrumbLabel="ביטוח בריאות"
      keyPoints={keyPoints}
      coverageTypes={coverageTypes}
      coverageSubtitle="מגוון רחב של כיסויים המותאמים לצרכים שלכם"
      articles={articles}
      faqItems={faqItems}
      companies={["הראל", "מנורה מבטחים", "מגדל", "כלל", "איילון", "הפניקס", "מיטב", "מור", "ילין לפידות", "אנליסט", "אינפיניטי", "אלטשולר שחם", "פאספורטקארד", "הכשרה"]}
      insuranceType="health"
      enrollmentTitle="הצטרפות לביטוח בריאות"
      enrollmentDescription="מלאו את הפרטים לקבלת הצעה מותאמת אישית"
    />
  );
};

export default HealthInsurance;
