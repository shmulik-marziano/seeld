  ...
  return (
    <div>
      <div className="mb-2 flex items-baseline justify-between">
        <label className="text-[13px] font-medium text-[#3f3f46]">{label}</label>
        <span className="tnum v2-display text-[15px] font-bold text-[#18181b]">{display}</span>
      </div>
      <input
        type="range"
        min={min}
...
function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      className={`flex items-center justify-between rounded-2xl border px-3.5 py-2.5 text-right transition-all ${
        checked
          ? "border-[#1d6bf3]/40 bg-[#1d6bf3]/8 text-[#18181b]"
          : "border-[#18182812] bg-[#f7f7f9] text-[#71717a] hover:border-[#18182824]"
      }`}
    >
      <span className="text-[13px] font-medium">{label}</span>
      <span className={`relative h-5 w-9 rounded-full transition-colors ${checked ? "bg-[#1d6bf3]" : "bg-[#d4d4d8]"}`}>
        <motion.span
          layout
...
function Stat({
  ...
  const accent = tone === "warn" ? "text-[#dc2626]" : tone === "good" ? "text-[#16a34a]" : "text-[#1d6bf3]";
  return (
    <GlassCard className="p-4">
      <div className={`mb-2 inline-flex h-8 w-8 items-center justify-center rounded-lg bg-[#f4f4f6] ${accent}`}>{icon}</div>
      <p className="text-[12px] text-[#71717a]">{label}</p>
      <p className="tnum v2-display mt-0.5 text-xl font-extrabold text-[#18181b]">{value}</p>
...
export default function ReportStudio() {
  ...
  const handlePdf = async () => {
    ...
    setExporting(true);
    try {
      await exportElementToPdf(reportRef.current, "seeld-financial-report.pdf", "#ececef");
    ...
    <V2Shell>
      <div className="mx-auto max-w-7xl px-5 sm:px-8 pt-10 sm:pt-14 pb-20">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="mb-8">
          ...
          <h1 className="v2-display mt-4 text-4xl sm:text-5xl font-extrabold leading-[1.05] text-[#18181b]">
            בנה את הדוח הפיננסי <GradientText>שלך</GradientText>
          </h1>
          <p className="mt-3 max-w-2xl text-[15px] sm:text-base text-[#52525b] leading-relaxed">
        ...
        <div className="grid lg:grid-cols-[380px_1fr] gap-6">
          ...
          <div className="lg:sticky lg:top-24 lg:self-start">
            <GlassCard className="p-5 sm:p-6">
              <h2 className="mb-5 text-[12px] font-bold uppercase tracking-wider text-[#a1a1aa]">הפרופיל שלך</h2>
              ...
              <h2 className="mb-3 mt-7 text-[12px] font-bold uppercase tracking-wider text-[#a1a1aa]">כיסוי ביטוחי</h2>
          ...
          <div ref={reportRef} className="rounded-[28px]">
            {/* Hero result */}
            <GlassCard className="relative overflow-hidden p-6 sm:p-8">
              <div className="pointer-events-none absolute -top-24 -left-24 h-60 w-60 rounded-full bg-[#1d6bf3]/10 blur-3xl" />
              <div className="relative flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-[13px] text-[#71717a]">צבירה צפויה בגיל {profile.retirementAge}</p>
                  <motion.p
                    key={Math.round(result.balanceAtRetirement)}
                    initial={{ opacity: 0.4, y: 6 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="v2-display tnum mt-1 text-5xl sm:text-6xl font-extrabold leading-none text-[#18181b]"
                  >
                    {shekels(result.balanceAtRetirement)}
                  </motion.p>
                  <p className="mt-3 text-[13px] text-[#52525b]">
                    מתוכם <span className="font-semibold text-[#1d6bf3]">{shekelsCompact(result.totalGrowth)}</span> תשואה ·
                    קצבה חודשית של <span className="font-semibold text-[#18181b]">{shekels(result.monthlyPension)}</span>
                  </p>
                </div>
                <div className="flex flex-col items-center">
                  <Ring value={result.readinessScore} size={132}>
                    <span className="v2-display tnum text-3xl font-extrabold text-[#18181b]">{result.readinessScore}</span>
                    <span className="text-[10px] text-[#a1a1aa]">מתוך 100</span>
                  </Ring>
                  <span
                    className={`mt-2 rounded-full px-3 py-1 text-[12px] font-semibold ${
                      ready.tone === "good"
                        ? "bg-[#16a34a]/12 text-[#15803d]"
                        : ready.tone === "ok"
            ...
            <GlassCard className="mt-4 p-5 sm:p-6">
              <div className="mb-3 flex items-center justify-between">
                <h3 className="text-[14px] font-bold text-[#18181b]">מסלול הצבירה עד הפרישה</h3>
                <div className="flex items-center gap-4 text-[11px]">
                  <span className="flex items-center gap-1.5 text-[#71717a]"><span className="h-2 w-3 rounded-full bg-[#1d6bf3]" /> תרחיש נוכחי</span>
                  <span className="flex items-center gap-1.5 text-[#71717a]"><span className="h-2 w-3 rounded-full bg-[#16a34a]" /> דמי ניהול מופחתים</span>
                </div>
              </div>
              <div className="h-64">
            ...
            {result.coverageGaps.length > 0 && (
              <GlassCard className="mt-4 p-5 sm:p-6">
                <h3 className="mb-3 flex items-center gap-2 text-[14px] font-bold text-[#18181b]">
                  <ShieldAlert className="h-4 w-4 text-[#dc2626]" /> פערים בכיסוי שכדאי לבחון
                </h3>
                <div className="grid sm:grid-cols-2 gap-2.5">
                  {result.coverageGaps.map((g) => (
                    <div key={g.key} className="rounded-2xl border border-[#18182810] bg-[#f7f7f9] p-3">
                      <div className="flex items-center gap-2">
                        <span
                          className={`h-2 w-2 rounded-full ${
                            g.severity === "high" ? "bg-[#dc2626]" : g.severity === "medium" ? "bg-[#d97706]" : "bg-[#0284c7]"
                          }`}
                        />
                        <p className="text-[13px] font-semibold text-[#18181b]">{g.label}</p>
                      </div>
                      <p className="mt-1 text-[12px] leading-relaxed text-[#71717a]">{g.reason}</p>
                    </div>
                  ))}
                </div>
              </GlassCard>
            ...
            <GlassCard className="mt-4 p-5 sm:p-6">
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                <h3 className="flex items-center gap-2 text-[14px] font-bold text-[#18181b]">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="v2-btn-secondary inline-flex items-center gap-2 px-4 py-2 text-[13px] font-semibold disabled:opacity-60"
                  >
                    {aiState === "streaming" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Sparkles className="h-3.5 w-3.5 text-[#1d6bf3]" />}
                    נתח עם אריק (AI)
                  </button>
                  <button
                    onClick={handlePdf}
                    disabled={exporting}
                    className="v2-btn-primary inline-flex items-center gap-2 px-4 py-2 text-[13px] font-bold disabled:opacity-60"
                  >
              ...
              <div
                className="max-w-none text-[14px] leading-[1.8] text-[#3f3f46]
                  [&_h2]:v2-display [&_h2]:mt-5 [&_h2]:mb-2 [&_h2]:text-[15px] [&_h2]:font-bold [&_h2]:text-[#18181b]
                  [&_strong]:font-bold [&_strong]:text-[#1d6bf3]
                  [&_ol]:mt-2 [&_ol]:space-y-1 [&_ol]:pr-5 [&_ol]:list-decimal
                  [&_ul]:mt-2 [&_ul]:space-y-1 [&_ul]:pr-5 [&_ul]:list-disc
                  [&_p]:mt-2"
              >
                <ReactMarkdown>{insights}</ReactMarkdown>
                {aiState === "streaming" && <span className="ml-1 inline-block h-4 w-1.5 animate-pulse bg-[#1d6bf3] align-middle" />}
              </div>

              {/* CTA */}
              <div className="mt-6 flex flex-col items-start gap-3 rounded-2xl border border-[#1d6bf3]/15 bg-[#1d6bf3]/[0.05] p-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-3">
                  <CheckCircle2 className="h-6 w-6 shrink-0 text-[#16a34a]" />
                  <p className="text-[13px] text-[#3f3f46]">רוצה לממש את ההזדמנויות שזוהו? פגישת הייעוץ הראשונה על חשבוננו.</p>
                </div>
                <Link to="/contact" className="v2-btn-primary inline-flex shrink-0 items-center gap-2 px-5 py-2.5 text-[13px] font-bold">
