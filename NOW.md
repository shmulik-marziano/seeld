...
**מובייל, רמת גימור, היררכיית מוצרים ויישור למוקאפים בענף `feature/mobile-craft`** (פתיחה עם קומפוזיציית עיגולים, פאנלי מוצרים, ציר זמן, תפריט קצר; נספח ב-`STYLESEED.md`) (מעל לוח התשואות, ממתין לאישור): סדר המוצרים לכל האתר ב-`src/data/productDirectory.ts` (פיננסים, חיים ובריאות, כללי). כפתורים צפים קומפקטיים בטלפון, תיקון גלישה ועימוד במסלולי השקעה, כרטיסי שירות קומפקטיים בבית, טקסט 14 פיקסל מינימום, יעדי מגע 44, איורי קיט במקום תמונות סטוק בבלוג, `api/geo.ts` במקום שירות מיקום חיצוני. כלי סקר: `scripts/mobile_audit.mjs`. דוח + תוכנית "צוות עיצוב": `docs/history/2026-09-17-mobile-and-craft.md`.
...
**האזור האישי החכם (17.9):** מחובר לאינסבייס (insbase.io) בזרימת הרשאה מהדפדפן; `src/lib/insbase.ts`, טבלה `insbase_links`; מבנה מסכי האפליקציה (בית, תיק, פניות, פרופיל). דוח: חלק ז ב-`docs/history/2026-09-17-mobile-and-craft.md`. שרת אינסבייס מקבל כרגע רק את רשומת הניסיון.

**כרטיסי מוצרים מובנים בתיק — חיים (18.9):** `ProductCards.tsx`, `FileTab.tsx`, `insbase.ts` קורא `structuredContent` (קומיט 8684a63 ב-`main`). שרת אינסבייס 0.9.2 מחזיר `structuredContent.products` על `list_products` (אומת מול רשומת הניסיון: 7 מוצרים). כשאין מבנה — הטקסט מוצג כרגיל. תיעוד: `חיבור לקוח לצ'אט/07_גרעין/תוספת-תשובה-מובנית-לאתר.md`.

**המערכת של הסוכנות (qlio, `C:\dev\insbase`):** חדר 26 — מערכת העיצוב הוחלה על כל המסכים (18.9). ראו `PROGRESS.md` שם.
