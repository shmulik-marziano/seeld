...
**לא מחויב, לא נבנה (17.9 לילה, שורת הפקודה נחסמה בסשן):** כרטיסי מוצרים מובנים בתיק (`ProductCards.tsx`, `FileTab.tsx`, `insbase.ts` קורא `structuredContent`). דורש: (א) `npm run build` + `tsc` בסשן חדש; (ב) התוספת לשרת אינסבייס לפי `חיבור לקוח לצ'אט/07_גרעין/תוספת-תשובה-מובנית-לאתר.md` (על הגרסה החיה 0.9.1, לא על הקובץ המקומי 0.6.0). עד אז הכרטיסים לא מופיעים והטקסט מוצג כרגיל.
